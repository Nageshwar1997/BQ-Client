...
/**
 * `withLiveCamera`'s exact counterpart for segmentation-tracked categories - same webcam
 * (getUserMedia + `requestAnimationFrame` render loop) shape, wired to `ImageSegmenter` instead of
 * `FaceLandmarker`. One real difference: the per-frame detection call uses `segmentForVideo`'s
 * *callback* overload, not the synchronous one `withLiveCamera` uses for `detectForVideo` -
 * `ImageSegmenterResult`'s own docs warn the synchronous overload copies every mask on every call,
 * "should not be used in high-throughput applications" (exactly what Live mode's per-frame loop
 * is) - the callback overload avoids that copy for masks read within it (see
 * `extractHairMask`'s own comment on why only the `Float32Array` extracted inside the callback,
 * not the mask/result objects themselves, is safe to keep past it).
 */
export function withLiveCameraSegmentation<TState extends IMakeupState, TAssets>(
  ...
  abstract class SegmentationLiveEngine extends Base {
    ...
    protected onTryOnReady() {
      // Same as `withLiveCamera` - the camera itself is started/stopped by the React wrapper's
      // imperative `startCamera`/`stopCamera`, not automatically here.
    }
    ...
    private loop = (token: number) => {
      ...
      try {
        this.segmenter.segmentForVideo(video, performance.now(), (result) => {
          // The callback can fire after a stop/restart already moved past this exact loop
          // iteration (camera stopped, video swapped) - re-check both guards the outer call
          // already checked, since neither is re-verified between scheduling this call and its
          // callback actually firing.
          if (!this.ensureAlive() || token !== this.restartToken) return;
          // `false`: this mask is task-owned (handed to us inside the callback), freed
          // automatically once this callback returns - see `extractHairMask`'s own comment.
  ...
  return SegmentationLiveEngine as unknown as new (
    canvas1: HTMLCanvasElement,
    canvas2: HTMLCanvasElement,
    initialState?: Partial<TState>,
  ) => ITryOnLiveEngineRef<TState>;
