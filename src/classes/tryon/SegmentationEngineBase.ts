...
import { getSharedHairSegmenter } from './HairSegmenterCache';

/**
 * `TryOnEngineBase`'s exact counterpart for segmentation-tracked categories - same state pub-sub,
 * abort-safe lifecycle, canvas sizing, compare-slider split-screen render, and snapshot capture,
 * just built on MediaPipe `ImageSegmenter` (a per-pixel confidence mask) instead of
 ...
 * abstraction), and `IRenderTargetParams`'s base shape carries a landmark-only `face` field. HAIR
 * is the only category needing this today - see docs/tryons/HAIR-PLAN.md's "Engine architecture"
 * section for the full reasoning, including why this duplicates rather than generalizes the
 * existing base.
 *
 ...
 */
export abstract class SegmentationEngineBase<TState extends IMakeupState, TAssets = null> {
  ...
  constructor(
    ...
    // Same reasoning as `TryOnEngineBase`'s identical block - a color seeded straight through
    // `initialState` (switching Live<->Upload with a shade already applied) never goes through
    // `updateState.set`, so `renderFrame` would silently no-op without this.
    if (this.state.color) {
  ...
  public updateState = {
    ...
    // Reused as-is from `TryOnEngineBase` - the field is still literally named `faceDetection`
    // (`IMakeupBaseState`, shared by every category's state), just repurposed here to mean "hair
    // region confidently detected" instead of "face" (see `getHairDetectionStatus`). Guarded the
    // same way, so a run of frames reporting the same status doesn't re-render React on every one.
  ...
  public async startTryOn(): Promise<void> {
    ...
    try {
      ...
      // Same "don't call cleanup()" reasoning as `TryOnEngineBase.startTryOn` - this engine is
      // still mounted and still subscribed-to by the React wrapper.
      this.segmenter = null;
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    this.updateState.setFaceDetectionStatus(getHairDetectionStatus(this.mask));
  ...
  protected cleanup() {
    ...
    // `HairSegmenterCache`, reused by later mounts (same reasoning as `TryOnEngineBase.cleanup`
    // not closing `this.landmarker`).
    this.segmenter = null;
    this.mask = null;
    this.updateState.internalReset();
  }
