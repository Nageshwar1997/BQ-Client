...
export class AddressApi extends ApiRequest {
  protected get_user_addresses = () => {
    return this.request(this.routes.addresses.get, { isPrivateRoute: true });
  };
  protected add_address = (data: TAddress) => {
    return this.request({ ...this.routes.addresses.add, data }, { isPrivateRoute: true });
  };
  protected update_address = (data: Partial<IAddress> & { removedOptionalFields?: string[] }) => {
    ...
    return this.request({ method, url: `${url}/${_id}`, data: restBody }, { isPrivateRoute: true });
  };
  protected delete_address = (id: string) => {
    const { method, url } = this.routes.addresses.delete;
    return this.request({ method, url: `${url}/${id}` }, { isPrivateRoute: true });
  };
}
