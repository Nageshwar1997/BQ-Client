...
import type { IBlushState, ITryOnLiveRef } from '../../../../../types';
import { BlushLiveClass } from '../../../classes/Blush/BlushLiveClass';

interface Props {
  initialState?: Partial<IBlushState>;
  onStateChange?: (state: IBlushState) => void;
  className?: string;
}

const BlushLive = forwardRef<ITryOnLiveRef, Props>(
  ({ initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const videoRef = useRef<HTMLVideoElement>(null!);

    const tryOnRef = useRef<BlushLiveClass | null>(null);
    ...
    useImperativeHandle(
      ref,
      () => ({
        setMakeupState: (s) => tryOnRef.current?.updateState.set(s as IBlushState),
    ...
    useEffect(() => {
      const tryOn = new BlushLiveClass(
        canvas1Ref.current,
        canvas2Ref.current,
        videoRef.current,
        initialState,
