...
import type { IBlushState, ITryOnUploadRef } from '../../../../../types';
import { BlushUploadClass } from '../../../classes/Blush/BlushUploadClass';
...
const BlushUpload = forwardRef<ITryOnUploadRef, Props>(
  ({ uploadedImageUrl, initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const tryOnRef = useRef<BlushUploadClass | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        setMakeupState: (s) =>
          tryOnRef.current?.updateState.set(s as IBlushState),
    ...
    useEffect(() => {
      const tryOn = new BlushUploadClass(
        canvas1Ref.current,
        canvas2Ref.current,
        initialState
      );
