...
import type { IEyeshadowState, ITryOnUploadRef } from '../../../../../types';
import { EyeshadowUploadClass } from '../../../classes/Eyeshadow/EyeshadowUploadClass';
...
const EyeshadowUpload = forwardRef<ITryOnUploadRef, Props>(
  ({ uploadedImageUrl, initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const tryOnRef = useRef<EyeshadowUploadClass | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        setMakeupState: (s) =>
          tryOnRef.current?.updateState.set(s as IEyeshadowState),
    ...
    useEffect(() => {
      const tryOn = new EyeshadowUploadClass(
        canvas1Ref.current,
        canvas2Ref.current,
        initialState
      );
...
export default EyeshadowUpload;
