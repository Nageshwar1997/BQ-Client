...
import type { IFoundationState, ITryOnLiveRef } from '../../../../../types';
import { FoundationLiveClass } from '../../../classes/Foundation/FoundationLiveClass';
...
const FoundationLive = forwardRef<ITryOnLiveRef, Props>(
  ({ initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const videoRef = useRef<HTMLVideoElement>(null!);

    const tryOnRef = useRef<FoundationLiveClass | null>(null);
    ...
    useImperativeHandle(
      ref,
      () => ({
        setMakeupState: (s) => tryOnRef.current?.updateState.set(s as IFoundationState),
    ...
    useEffect(() => {
      const tryOn = new FoundationLiveClass(
        canvas1Ref.current,
        canvas2Ref.current,
        videoRef.current,
        initialState
