...
import type { ILipstickState, ITryOnUploadRef } from '../../../../../types';
import { LipstickUploadClass } from '../../../classes/Lipstick/LipstickUploadClass';

interface Props {
  uploadedImageUrl?: string | null;
  initialState?: Partial<ILipstickState>;
  onStateChange?: (state: ILipstickState) => void;
  className?: string;
}

const LipstickUpload = forwardRef<ITryOnUploadRef, Props>(
  ({ uploadedImageUrl, initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const tryOnRef = useRef<LipstickUploadClass | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        setMakeupState: (s) =>
          tryOnRef.current?.updateState.set(s as ILipstickState),
    ...
    useEffect(() => {
      const tryOn = new LipstickUploadClass(
        canvas1Ref.current,
        canvas2Ref.current,
        initialState
      );
