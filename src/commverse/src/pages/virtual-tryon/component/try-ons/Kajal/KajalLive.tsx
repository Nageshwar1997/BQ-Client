...
import type { IKajalState, ITryOnLiveRef } from '../../../../../types';
import { KajalLiveClass } from '../../../classes/Kajal/KajalLiveClass';
...
const KajalLive = forwardRef<ITryOnLiveRef, Props>(
  ({ initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const videoRef = useRef<HTMLVideoElement>(null!);

    const tryOnRef = useRef<KajalLiveClass | null>(null);
    ...
    useImperativeHandle(
      ref,
      () => ({
        setMakeupState: (s) => tryOnRef.current?.updateState.set(s as IKajalState),
    ...
    useEffect(() => {
      const tryOn = new KajalLiveClass(
        canvas1Ref.current,
        canvas2Ref.current,
        videoRef.current,
        initialState,
