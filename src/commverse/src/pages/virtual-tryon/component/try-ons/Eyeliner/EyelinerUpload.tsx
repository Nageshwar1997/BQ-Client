...
import type { IEyelinerState, ITryOnUploadRef } from '../../../../../types';
import { EyelinerUploadClass } from '../../../classes/Eyeliner/EyelinerUploadClass';
...
const EyelinerUpload = forwardRef<ITryOnUploadRef, Props>(
  ({ uploadedImageUrl, initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const tryOnRef = useRef<EyelinerUploadClass | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        setMakeupState: (s) =>
          tryOnRef.current?.updateState.set(s as IEyelinerState),
    ...
    useEffect(() => {
      const tryOn = new EyelinerUploadClass(
        canvas1Ref.current,
        canvas2Ref.current,
        initialState
      );
