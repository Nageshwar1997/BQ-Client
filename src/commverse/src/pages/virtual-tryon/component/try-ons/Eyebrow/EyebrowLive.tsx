...
import type { IEyebrowState, ITryOnLiveRef } from '../../../../../types';
import { EyebrowLiveClass } from '../../../classes/Eyebrow/EyebrowLiveClass';

interface Props {
  initialState?: Partial<IEyebrowState>;
  onStateChange?: (state: IEyebrowState) => void;
  className?: string;
}

const EyebrowLive = forwardRef<ITryOnLiveRef, Props>(
  ({ initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const videoRef = useRef<HTMLVideoElement>(null!);

    const tryOnRef = useRef<EyebrowLiveClass | null>(null);
    ...
    useImperativeHandle(
      ref,
      () => ({
        setMakeupState: (s) => tryOnRef.current?.updateState.set(s as IEyebrowState),
    ...
    useEffect(() => {
      const tryOn = new EyebrowLiveClass(
        canvas1Ref.current,
        canvas2Ref.current,
        videoRef.current,
        initialState,
