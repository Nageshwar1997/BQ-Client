...
import type { IEyebrowState, ITryOnUploadRef } from '../../../../../types';
import { EyebrowUploadClass } from '../../../classes/Eyebrow/EyebrowUploadClass';
...
const EyebrowUpload = forwardRef<ITryOnUploadRef, Props>(
  ({ uploadedImageUrl, initialState, onStateChange, className }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null!);
    const canvas2Ref = useRef<HTMLCanvasElement>(null!);
    const tryOnRef = useRef<EyebrowUploadClass | null>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        setMakeupState: (s) =>
          tryOnRef.current?.updateState.set(s as IEyebrowState),
    ...
    useEffect(() => {
      const tryOn = new EyebrowUploadClass(
        canvas1Ref.current,
        canvas2Ref.current,
        initialState
      );
