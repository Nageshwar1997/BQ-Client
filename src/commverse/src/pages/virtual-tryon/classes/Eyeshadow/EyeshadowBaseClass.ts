...
import { left_eyeshadow_indices, right_eyeshadow_indices } from '../../data';

export abstract class EyeshadowBaseClass {
  protected state: IEyeshadowState = getInitialState(
    'Eyeshadow'
  ) as IEyeshadowState;
  ...
  protected listeners: ((state: IEyeshadowState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IEyeshadowState>) => {
    ...
    internalReset: () => {
      this.state = getInitialState('Eyeshadow') as IEyeshadowState;
  ...
  public onChange(listener: (state: IEyeshadowState) => void) {
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const applyMakeup = () => {
      if (
        ...
        return;
      const indices = {
        left: left_eyeshadow_indices,
        right: right_eyeshadow_indices,
      };
      applyEyeShadow(
        face,
        ctx,
        color,
        indices,
        this.textureImages.Ls, // new shimmer
        size
      );
    };
