...
export abstract class EyelinerBaseClass {
  protected state: IEyelinerState = getInitialState(
    'Eyeliner'
  ) as IEyelinerState;
  ...
  protected listeners: ((state: IEyelinerState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IEyelinerState>) => {
    ...
    internalReset: () => {
      this.state = getInitialState('Eyeliner') as IEyelinerState;
  ...
  public onChange(listener: (state: IEyelinerState) => void) {
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const applyMakeup = () => {
      ...
      const eyePoints = getEyelinerIndices(this.state.type);
