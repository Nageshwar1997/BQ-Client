...
import type { ColorTuple, IKajalState, TRunningMode } from '../../../../types';

export abstract class KajalBaseClass {
  protected state: IKajalState = getInitialState('Kajal') as IKajalState;
  ...
  protected listeners: ((state: IKajalState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IKajalState>) => {
    ...
    internalReset: () => {
      this.state = getInitialState('Kajal') as IKajalState;
  ...
  public onChange(listener: (state: IKajalState) => void) {
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const color = `rgba(${r},${g},${b},${this.state.range})`;
    const size = { width, height };

    const applyMakeup = () => {
      if (!this.state.type) return;
      const eyePoints = getKajalIndices(this.state.type);
      if (eyePoints.left.length && eyePoints.right.length) {
        applyTextureOnEyes(
          face,
          ctx,
          this.state.type,
