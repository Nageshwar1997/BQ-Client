...
import type { ColorTuple, IBlushState, TRunningMode } from '../../../../types';

export abstract class BlushBaseClass {
  protected state: IBlushState = getInitialState('Blush') as IBlushState;
  ...
  protected textureImages: Record<'L' | 'U' | 'Lc' | 'Uc' | 'Ls' | 'Us', HTMLImageElement> | null =
    null;

  protected cachedRGBA: ColorTuple | null = null;
  protected listeners: ((state: IBlushState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IBlushState>) => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { cameraReady, imageReady, tryOnStarted, error, ...safePartial } = partial;
    ...
    internalReset: () => {
      this.state = getInitialState('Blush') as IBlushState;
  ...
  public onChange(listener: (state: IBlushState) => void) {
  ...
  public async startTryOn(): Promise<void> {
    ...
    try {
      ...
      this.landmarker = await getLandmarker(this.canvas1, this.getRunningMode(), signal);
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const applyMakeup = () => {
      addBlushEffect(ctx, face, size, [r, g, b, this.state.range], 50, 280);
      addBlushEffect(ctx, face, size, [r, g, b, this.state.range], 123, 352);
      addBlushEffect(ctx, face, size, [r, g, b, this.state.range], 116, 345);
    };
