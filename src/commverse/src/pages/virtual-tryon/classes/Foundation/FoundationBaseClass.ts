...
export abstract class FoundationBaseClass {
  protected state: IFoundationState = getInitialState(
    'Foundation'
  ) as IFoundationState;
  ...
  protected listeners: ((state: IFoundationState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IFoundationState>) => {
    ...
    internalReset: () => {
      this.state = getInitialState('Foundation') as IFoundationState;
  ...
  public onChange(listener: (state: IFoundationState) => void) {
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const applyMakeup = () => {
      const faceRegion = extractFaceRegion(face, size, ctx);
      //const faceRegion1 = extractFaceRegion1(face, size);
      const leftEyeRegion = extractRegion(face, size, 'left-eye');
      const rightEyeRegion = extractRegion(face, size, 'right-eye');
      const lipsRegion = extractRegion(face, size, 'lips');
      ...
      if (faceRegion) {
        applyTintEffect(
          ...
          [leftEyeRegion, rightEyeRegion, lipsRegion],
          [r, g, b, this.state.range ?? 0]
        );
      }
    };
