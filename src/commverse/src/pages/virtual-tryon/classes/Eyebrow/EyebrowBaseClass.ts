...
import { left_eyebrow_indices, right_eyebrow_indices } from '../../data';

export abstract class EyebrowBaseClass {
  protected state: IEyebrowState = getInitialState('Eyebrow') as IEyebrowState;
  ...
  protected listeners: ((state: IEyebrowState) => void)[] = [];
  // lifecycle guards
  ...
  public updateState = {
    set: (partial: Partial<IEyebrowState>) => {
    ...
    internalReset: () => {
      this.state = getInitialState('Eyebrow') as IEyebrowState;
  ...
  public onChange(listener: (state: IEyebrowState) => void) {
  ...
  protected renderFrame(drawSource: HTMLVideoElement | HTMLImageElement) {
    ...
    const color: ColorTuple = [r, g, b, this.state.range];
    const size = { width, height };

    const applyMakeup = () => {
      const indices = {
        left: left_eyebrow_indices,
        right: right_eyebrow_indices,
      };
      applyEyeBrow(face, ctx, color, indices, size);
    };
