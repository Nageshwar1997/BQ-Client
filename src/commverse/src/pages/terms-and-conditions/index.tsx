...
const renderInfoValue = ({ value, href }: TermsInfoItem) => {
  if (!href) {
    return value;
  }
...
const renderBlock = (block: TermsBlock, key: string) => {
  if (block.type === 'list') {
    return (
      <ul
        key={key}
...
const TermsSectionView = ({
  ...
  <section className={level === 0 ? 'space-y-5' : 'space-y-4'}>
    ...
    {section.subsections?.length ? (
      <div className="space-y-8 border-l border-[rgba(0,45,255,0.08)] pl-4 md:pl-6">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-8 sm:px-8 md:px-12 md:py-12">
        ...
        <div className="mt-10 space-y-10 md:mt-14">
          ...
          <footer className="border-brand/10 space-y-3 border-t pt-6">
            {termsClosingNotes.map((note, index) => (
              <p
                key={note}
