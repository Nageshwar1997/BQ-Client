...
const RightContainer = ({ settings, onSettingChange }: RightContainerProps) => {
  ...
  const handleConfirmSelectedModel = async () => {
    ...
    try {
      ...
      navigate(`/3d-visualizer/${experienceId}`);
      setIsSelectLibraryOpen(false);
    } catch (error) {
      showToast(
        'error',
      ...
      console.error('Failed to create 3d visualizer experience:', error);
    }
  };
  ...
  return (
    <>
      <div className="font-metropolis text-neutral-gray-900 flex flex-1 flex-col items-center justify-center gap-6 px-8 py-6">
