...
import { HairUploadEngine } from '@/classes/tryon/categories/hair';
import type { ITryOnStageRef } from '@/types/tryon-types';

interface IHairUploadStageProps {
  imageUrl: string | null;
  initialState?: Partial<IHairTryOnState>;
  onStateChange: (state: IHairTryOnState) => void;
}

// Mirrors ../face/FaceUploadStage.tsx exactly, just wired to HairUploadEngine.
const HairUploadStage = forwardRef<ITryOnStageRef<IHairTryOnState>, IHairUploadStageProps>(
  ({ imageUrl, initialState, onStateChange }, ref) => {
    const canvas1Ref = useRef<HTMLCanvasElement>(null);
    const canvas2Ref = useRef<HTMLCanvasElement>(null);
    const engineRef = useRef<HairUploadEngine | null>(null);
    ...
    useEffect(() => {
      ...
      const engine = new HairUploadEngine(canvas1, canvas2, initialState);
...
HairUploadStage.displayName = 'HairUploadStage';

export default HairUploadStage;
